...
def run_single_config(init_temp, cooling_rate, step_size, num_steps):
    ...
    avg_reward, costs, _, _ = run_sa(
        init_temp, cooling_rate, step_size,
...
def process_config(temp, cool, step, count, total):
    ...
    result = {
        'temp': temp, 'cool': cool,
        'step': step, 'mean': mean_cost,
        'std': std_cost
    }
    
    print(f"[{count}/{total}] T={temp:.1f}, "
...
def save_results(best_params, best_cost, results):
    ...
    with open('outputs/grid_search_results.txt', 'w') as f:
        ...
        f.write("All Results:\n")
        for r in results:
            f.write(f"T={r['temp']:.1f}, α={r['cool']:.2f}, "
