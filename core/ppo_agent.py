...
class PPOAgent:
    def __init__(
        self, state_dim, action_dim, lr=3e-4,
    ...
    def _compute_returns(self, rewards, is_terminals):
        ...
        for reward, terminal in zip(
            reversed(rewards), reversed(is_terminals)
        ):
            if terminal:
                discounted = 0
    ...
    def _compute_loss(
        self, dist, state_values, old_log_probs,
        old_actions, returns
    ):
        """Compute PPO loss."""
    ...
    def update(self, memory):
        ...
        old_log_probs = torch.FloatTensor(
            np.array(memory['log_probs'])
        )
        
        returns = self._compute_returns(
            memory['rewards'], memory['is_terminals']
        )
        ...
        for _ in range(self.k_epochs):
            ...
            loss = self._compute_loss(
                dist, state_values, old_log_probs,
                old_actions, returns
            )
